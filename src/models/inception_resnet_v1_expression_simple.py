...
def inception_resnet_v1_expression(inputs, is_training=True,
                        dropout_keep_prob=1.0,
                        reuse=None,
                        scope='InceptionResnetV1_expression'):
    """Creates the Inception Resnet V1 model.
    ...
    with tf.variable_scope(scope, 'InceptionResnetV1_expression', [inputs], reuse=reuse):
        with slim.arg_scope([slim.batch_norm, slim.dropout],
                            is_training=is_training):
            with slim.arg_scope([slim.conv2d, slim.fully_connected],
                ...
                # 3x3x128x256
                net = slim.conv2d(net, 256, 3, scope='Conv2d_3a_3x3')
    ...
    with tf.variable_scope(scope, 'InceptionResnetV1', [inputs], reuse=reuse):
        with slim.arg_scope([slim.batch_norm, slim.dropout],
            ...
            with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
                ...
                with tf.variable_scope('_sLogits'):
                    end_points['PrePool'] = net
                    # pylint: disable=no-member
                    net = slim.avg_pool2d(net, net.get_shape()[1:3], padding='VALID',
                                          scope='AvgPool_1a_8x8')
